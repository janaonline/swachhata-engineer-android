...
//            mTracker = analytics.newTracker(getString(R.string.tracking_id));
    ...
    public static void assignLanguage(final Activity activity) {
        try {
            Locale locale = new Locale(SecurePrefManager.with(activity).get(ICMyCPreferenceData.selectedLanguage).defaultValue("en").go());
            Locale.setDefault(locale);
...
//            setEmptyViewForRecyclerView(activity, (EasyRecyclerView) activity.findViewById(R.id.mRecyclerview));
