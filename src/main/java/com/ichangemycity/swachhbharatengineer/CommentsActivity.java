    ...
    send.setOnClickListener(v -> {
      if (((EditText) findViewById(R.id.textComment)).getText().toString().trim().length() > 0) {
        ...
        AppUtils.showToast(activity, AppConstant.TOAST_TYPE_INFO,
            activity.getResources().getString(R.string.write_a_comment));
  ...
  private class InitiatePostComment extends AsyncTask<Void, Void, Void> {
    ...
    protected void onPostExecute(Void aVoid) {
      ...
      if (!TextUtils.isEmpty(AppController.mSelectedImageModels.getPathOfSelectedImage())) {
        ...
        if (!TextUtils
            .isEmpty(((EditText) findViewById(R.id.textComment)).getText().toString())) {
          postComment(false);
        } else
//                    Toast.makeText(activity, getResources().getString(R.string.write_a_comment), Toast.LENGTH_SHORT).show();
  ...
  private void postComment(final boolean hasImage) {
    ...
    String urlParams = "?complaintId=" + AppController.selectedComplaintData.getComplaintId() +
        ...
        "&commentDescription=" + ((EditText) findViewById(R.id.textComment)).getText().toString()
        .trim().replace(" ", "%20");
    if (hasImage) {
    ...
    new WebserviceHelper(activity, WebserviceHelper.METHOD_POST, url + urlParams, params,
...
//                            Toast.makeText(activity, responseJsonObject.optString("message"), Toast.LENGTH_SHORT).show();
  ...
  private void showSelectedImage() {
    if (!TextUtils.isEmpty(AppController.mSelectedImageModels.getPathOfSelectedImage())) {
      ...
      Glide.with(activity).load((AppController.mSelectedImageModels.getUriOfImage()))
          .into(imageToUpload);
    } else {
...
//                isLoadMore = true;
  ...
  private void showAlertToPickImage() {
    ...
    startActivity(new Intent(activity, SelectImageDialogActivity.class)
